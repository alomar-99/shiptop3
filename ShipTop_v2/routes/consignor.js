...
router.post("/addOrder",urlEncodedParser,(req,res)=>{
    ...
    for(let x in req.body.shipments){
        ...
        // shipmentSQL += "INSERT INTO shipmentrecord(shipmentID, recordedPlace, recordedTime, action, actor) VALUES((SELECT * from shipmentUpdate WHERE shipmentID = (SELECT MAX(shipmentID) FROM shipment)), 'ADD', " + req.body.employeeID + "); \n";
    }
    shipmentSQL += "COMMIT; "
