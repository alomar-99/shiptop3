...
const headCells = [
  ...
  { id: "origin_name", numeric: true, disablePadding: false, label: "Origin" },
];
...
export default function EnhancedTable() {
  ...
  const fetchData = async () => {
    ...
    const { data } = await Axios.get("/return", {
              ...
              {stableSort(dataTable, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  ...
                  return (
                    <TableRow
                      hover
                      onClick={(event) => handleClick(event, row.awb)}
                      role="checkbox"
                    ...
                    >
                      ...
                      <TableCell align="right">
                        {row.destination_address_line_1 || "-"}
                      </TableCell>
                      <TableCell align="right">{row.status || "-"}</TableCell>{" "}
                      <TableCell align="right">{row.updatedAt ? moment(row.updatedAt).format("yyyy-MM-DD") : '-'}</TableCell>
