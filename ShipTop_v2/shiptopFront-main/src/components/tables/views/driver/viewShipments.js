...
const EnhancedTableToolbar = (props) => {
  ...
  const { numSelected, selectedRows, data , fetchData} = props;
  ...
  const assignShipments = async () => {
    ...
    const body = JSON.stringify({
      shipmentID: selectedRows,
      employeeID: sessionStorage.getItem("userId"),
    });
    axios
      .post(`${config.API_ROOT}/api/driver/deliverShipments`, body, {
        headers: {
...
export default function EnhancedTable(props) {
  ...
  const fetchData = async (searchQ, filterV) => {
    ...
    axios
      .get(
        `${config.API_ROOT}/api/driver/viewShipments${
