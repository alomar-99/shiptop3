...
const EnhancedTableToolbar = (props) => {
  ...
  const { numSelected, selectedRows, data, setShipments,filter } = props;
  ...
  const assignShipments = () => {
    ...
    setShipments({filter: filter, shipment:[...selectedRows]});
  };
  return (
    <Toolbar
...
export default function EnhancedTable(props) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = dataTable.map((n) => n.shipmentID);
  ...
  const fetchData = async (filterV) => {
    ...
    var data = [];
    axios
      .post(`${config.API_ROOT}/api/logisticManager/viewShipments`, body, {
        headers: {
        ...
        if (Array.isArray(data)) {
          ...
          if (props.filters.length === 1) {
            finalizeData = finalizeData.filter(
              (item) =>
                item.currentCity.toUpperCase() ===
                item.shipmentLocation.toUpperCase()
            );
          } 
        }
  ...
  return (
    <div className={classes.root}>
      ...
      <Filter
        Filter={{ name: "Status", filters: [...props.filters] }}
        handleFilters={(filters) => fetchData(filters)}
      />
      <CreateOrder open={isOpen} onClose={() => setIsOpen(false)}></CreateOrder>
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell padding="checkbox">
                        <Checkbox
                          onChange={(event) =>
                            handleClick(event, row.shipmentID)
                          }
