...
export default function EnhancedTable({ Display, setWarehouse }) {
  const [dataTable, setDataTable] = useState([]);

  const classes = useStyles();
  const [order, setOrder] = React.useState("asc");
  const [orderBy, setOrderBy] = React.useState("managerID");
  ...
  const fetchData = async (searchQ, filterV) => {
    ...
    axios
      .get(
        `${config.API_ROOT}/api/logisticManager/viewWarehouses${
          ...
          >
            ...
            <TableBody>
              {stableSort(dataTable, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  const isItemSelected = isSelected(row.managerID);
                      ...
                      {Display === "View" ? (
                        <>
                          ...
                          <TableCell align="center" key="action">
                            <IconButton
                              aria-label="delete"
                              onClick={() =>
                                onToggleEditMode(row.managerID, "Edit")
                              }
                            >
                              <DeleteIcon />
                            </IconButton>
                        ...
                        <TableCell align="center" key="action">
                          <Button
                            ...
                            className={classes.spacing}
                            onClick={() => {
                              console.log(`${row.managerID} in assing warehouse manage`)
                              setWarehouse(row.managerID)}}
                          >
                            Assign
                          </Button>
                        </TableCell>
