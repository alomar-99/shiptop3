...
export default function EnhancedTable(props) {
  ...
  const fetchData = async (filterV) => {
    ...
    var data = [];
    axios
      .post(`${config.API_ROOT}/api/worker/viewShipments`, body, {
        headers: {
      ...
      .then((res) => {
        ...
        if (props.filters[0] === "TOBESTORED"){
          finalizeData = finalizeData.filter((data) =>data.deliveryStatus === "TOBESTORED") 
        } else {

          finalizeData = finalizeData.filter((data) =>data.deliveryStatus === "PICKUP") 
        }
