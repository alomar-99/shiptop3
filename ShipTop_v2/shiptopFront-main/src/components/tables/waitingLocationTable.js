...
const headCells = [
  ...
  { id: "coord", numeric: false, disablePadding: false, label: "Coord", sort: false },
  {
    id: "status",
    numeric: false,
  ...
  { id: "pickuptime", numeric: true, disablePadding: false, label: "Picked At" },
...
export default function EnhancedTable() {
  ...
  const [orderBy, setOrderBy] = React.useState("awb");
  ...
  const fetchData = async (searchQ, filterV) => {
    ...
    const { data } = await Axios.get( `${config.API_ROOT}/waitingl${queryParams ? `?${queryParams}` : ""}`,
    ...
    if (Array.isArray(data)) {
      finalizeData = data.map((item, index) => ({
        ...
        coord: (item.locations && item.locations.length > 0) ? item.locations[0].coord : null,
        isEditMode: false,
      }))
    }
  ...
  const onToggleEditMode = (id, action) => {
    ...
    if (action == 'Save' && !currentObj["coord"]) {
      setSnackConfig({ open: true, message: "Coord is not allowed to be empty" })
    }
    else {
      setDataTable((state) => {
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <CustomTableCell row={{ ...row }} placeholder="00.00, 00.00" name="coord" onInputChange={(e, r) => onCustomCellChange(e, r)} />
                      ...
                      <TableCell align="right">{row.pickup_time}</TableCell>
                      <TableCell align="right">{row.origin_name}</TableCell>
                      <TableCell align="right">{row.destination_address_line_1}</TableCell>
